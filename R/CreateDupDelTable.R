...
#' @return
#' @export
CreateDupDelTable<-function(window, accession, vcf_path, output_path, isPlot=TRUE) {
  ...
  if(isPlot == TRUE) {
    ...
    PlotGenome(orderedTable = duplication_vcf_table,Window = window,Ylim = 3,PValue = "TRUE",Organism = "Human")
    dev.off()
  }
